...
impl Store {
    ...
    pub fn generate_url(&self, name: &str) -> String {
        format!("{}/{}/{}/{}", self.baseurl, &name[0..2], &name[2..4], name)
    }
}

#[test]
fn test_url() {
    let store = Store::new("", "https://examples.com/demos");
    assert_eq!(
        "https://examples.com/demos/ab/cd/abcdef",
        store.generate_url("abcdef")
    );

    let store = Store::new("", "https://examples.com/");
    assert_eq!(
        "https://examples.com/ab/cd/abcdef",
        store.generate_url("abcdef")
    );
}
